...
def run():
    try:
        index_id = Index.objects.filter(name="DJ30")[0]
    except:
        index_id = Index.objects.create(name="DJ30")

    Stock.objects.filter(index=index_id).delete()


    with open('scripts/dj.csv', 'r') as f:
        reader = csv.reader(f)
        next(reader)

        for row in reader:
            print(row)
            index = Index.objects.filter(name="DJ30")[0]
            ticker = row[0]
            name = row[1]
            ...
            s = Stock(ticker=ticker, name=name, momentum_12_2=mom_12_2,
                       momentum_avg=avg_mom, ma10=ma10, e_p=e_p, div_p=div_p, index=index)
            s.save()
